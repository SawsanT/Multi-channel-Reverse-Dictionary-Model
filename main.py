...
def main(frequency, batch_size, epoch_num, verbose, MODE):
    ...
    wd_sems = torch.from_numpy(np.array(wd_sems)) #torch.from_numpy(np.array(wd_sems[:label_size])).to(device)
    ...
    wd_lex = torch.from_numpy(np.array(wd_lex))
    ...
    wd_ra = torch.from_numpy(np.array(wd_ra))
