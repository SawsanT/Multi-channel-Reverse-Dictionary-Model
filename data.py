...
def load_data(frequency):
    ...
    data_train = json.load(open(os.path.join(data_path, 'data_train.json'), errors='replace'))
    data_dev = json.load(open(os.path.join(data_path, 'data_dev.json'), errors='replace'))
    data_test_500_rand1_seen = json.load(open(os.path.join(data_path, 'data_test_500_rand1_seen.json'), errors='replace'))
    data_test_500_rand1_unseen = json.load(open(os.path.join(data_path, 'data_test_500_rand1_unseen.json'), errors='replace')) #data_test_500_others
    data_defi_c = json.load(open(os.path.join(data_path, 'data_defi_c.json'), errors='replace'))
    data_desc_c = json.load(open(os.path.join(data_path, 'data_desc_c.json'), errors='replace'))
    ...
    vec_inuse = json.load(open(os.path.join(data_path, 'vec_inuse.json'), errors='replace'))
...
def my_collate_fn(batch):
    ...
    words_t = torch.tensor(np.array(words), dtype=torch.int64,)
    definition_words_t = torch.tensor(build_sentence_numpy(definition_words), dtype=torch.int64,)
...
def mask_noFeature(label_size, wd2fea, feature_num):
    mask_nofea = torch.zeros(label_size, dtype=torch.float32,)
